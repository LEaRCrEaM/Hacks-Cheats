this.wordle = this.wordle || {}, this.wordle.bundle = function (e) {
    ...
    CrEaM.textContent = 'Auto Insert Solution';
    CrEaM.addEventListener('click', function() {
        for (let i=0;i<5;i++) {
            const LEaR = document.querySelector('game-app').shadowRoot.querySelector('header .title').textContent.split('');
            const a = LEaR[i];
            document.querySelector('game-app').$keyboard.shadowRoot.querySelector(`[data-key="${a}"]`).click();
            if (i===4) {
                document.querySelector('game-app').$keyboard.shadowRoot.querySelectorAll(`[data-key]`)[19].click();
            }
        }
    })
            ...
            return o(t, [{
                key: "evaluateRow", value: function () {
                    if (5 === this.tileIndex && !(this.rowIndex >= 6)) {
                        ...
                        if (e = s, !WORD_LIST.includes(e) && !SOLUTION_LIST.includes(e)) return a.setAttribute("invalid", ""), void this.addToast(this.solution);
                        if (this.hardMode) {
                ...
                key: "connectedCallback", value: function () {
                    var e = this;
                    this.shadowRoot.appendChild(Ka.content.cloneNode(!0)), this.shadowRoot.querySelector('header .title').textContent = this.solution, this.$game = this.shadowRoot.querySelector("#game"), this.$board = this.shadowRoot.querySelector("#board"), this.$keyboard = this.shadowRoot.querySelector("game-keyboard"), this.sizeBoard(), this.lastPlayedTs || setTimeout((function () {
        ...
        return o(t, [{
            key: "connectedCallback", value: function () {
                ...
                if (["gamesPlayed", "winPercentage", "currentStreak", "maxStreak"].forEach((function (s) {
                    ...
                    this.shadowRoot.querySelector("button#refresh-button").click();
                    this.shadowRoot.querySelector("button#refresh-button").click()
                }
            }
        }]), t
